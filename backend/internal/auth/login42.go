...
func (auth *AuthService) getOrCreate42User(ctx context.Context, profile *FortyTwoUserProfile) (*ent.User, error) {
	...
	conv, cerr := auth.Client.Conversation.Query().
	...
	if cerr == nil {
		...
		conv, cerr = auth.Client.Conversation.Create().
			SetIsGroup(true).
			SetTitle("Global").
			Save(ctx)
		if cerr != nil {
			log.Print("failed to create global group:", cerr)
			return u, nil
		}
		globalID = conv.ID
	} else {
		log.Print("failed to query global group:", cerr)
		return u, nil
	}

	_, aerr := auth.Client.Conversation.UpdateOneID(globalID).
		AddParticipantIDs(u.ID).
		Save(ctx)
	if aerr != nil {
		log.Print("failed to add 42 user to global group:", aerr)
	} else {
		log.Printf("Ensured user %s is in Global chat!\n", u.Username)
	}
