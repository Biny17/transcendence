...
func (_q *FriendshipQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Friendship, error) {
	...
	if _q.withUser != nil || _q.withFriend != nil {
		withFKs = true
	}
	if withFKs {
...
func (_q *FriendshipQuery) loadFriend(ctx context.Context, query *UserQuery, nodes []*Friendship, init func(*Friendship), assign func(*Friendship, *User)) error {
	...
	nodeids := make(map[int][]*Friendship)
	for i := range nodes {
		if nodes[i].user_friend_of == nil {
			continue
		}
		fk := *nodes[i].user_friend_of
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_friend_of" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
