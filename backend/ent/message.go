...
func (*Message) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case message.ForeignKeys[0]: // conversation_messages
			values[i] = new(sql.NullInt64)
		case message.ForeignKeys[1]: // user_send_messages
			values[i] = new(sql.NullInt64)
		default:
...
func (_m *Message) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case message.ForeignKeys[1]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for edge-field user_send_messages", value)
			} else if value.Valid {
				_m.user_send_messages = new(int)
				*_m.user_send_messages = int(value.Int64)
			}
		default:
