...
func (_u *ConversationUpdate) AddParticipants(v ...*User) *ConversationUpdate {
	ids := make([]int, len(v))
	for i := range v {
		ids[i] = v[i].ID
	}
...
func (_u *ConversationUpdate) RemoveParticipants(v ...*User) *ConversationUpdate {
	ids := make([]int, len(v))
	for i := range v {
		ids[i] = v[i].ID
	}
...
func (_u *ConversationUpdate) sqlSave(ctx context.Context) (_node int, err error) {
	...
	if _u.mutation.ParticipantsCleared() {
		edge := &sqlgraph.EdgeSpec{
			...
			Columns: conversation.ParticipantsPrimaryKey,
			Bidi:    false,
			Target: &sqlgraph.EdgeTarget{
...
// AddParticipants adds the "participants" edges to the User entity.
func (_u *ConversationUpdateOne) AddParticipants(v ...*User) *ConversationUpdateOne {
	ids := make([]int, len(v))
	for i := range v {
		ids[i] = v[i].ID
	}
...
// RemoveParticipants removes "participants" edges to User entities.
func (_u *ConversationUpdateOne) RemoveParticipants(v ...*User) *ConversationUpdateOne {
	ids := make([]int, len(v))
	for i := range v {
		ids[i] = v[i].ID
	}
