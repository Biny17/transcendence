...
func (*Friendship) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case friendship.ForeignKeys[0]: // user_friendships
			values[i] = new(sql.NullInt64)
		case friendship.ForeignKeys[1]: // user_friend_of
			values[i] = new(sql.NullInt64)
		default:
	...
	for i := range columns {
		...
		case friendship.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for edge-field user_friendships", value)
			} else if value.Valid {
				_m.user_friendships = new(int)
				*_m.user_friendships = int(value.Int64)
			}
		case friendship.ForeignKeys[1]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for edge-field user_friend_of", value)
			} else if value.Valid {
				_m.user_friend_of = new(int)
				*_m.user_friend_of = int(value.Int64)
			}
		default:
