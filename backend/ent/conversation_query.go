...
func (_q *ConversationQuery) QueryParticipants() *UserQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2M, true, conversation.ParticipantsTable, conversation.ParticipantsPrimaryKey...),
		)
		fromU = sqlgraph.SetNeighbors(_q.driver.Dialect(), step)
		return fromU, nil
	}
...
func (_q *ConversationQuery) WithParticipants(opts ...func(*UserQuery)) *ConversationQuery {
	query := (&UserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ConversationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Conversation, error) {
	...
	if query := _q.withParticipants; query != nil {
		if err := _q.loadParticipants(ctx, query, nodes,
			func(n *Conversation) { n.Edges.Participants = []*User{} },
			func(n *Conversation, e *User) { n.Edges.Participants = append(n.Edges.Participants, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *ConversationQuery) loadParticipants(ctx context.Context, query *UserQuery, nodes []*Conversation, init func(*Conversation), assign func(*Conversation, *User)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Conversation)
	nids := make(map[int]map[*Conversation]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(conversation.ParticipantsTable)
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(conversation.ParticipantsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(conversation.ParticipantsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(conversation.ParticipantsPrimaryKey[1]))
