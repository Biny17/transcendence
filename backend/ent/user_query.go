...
// QueryMailVerif chains the current query on the "mail_verif" edge.
func (_q *UserQuery) QueryMailVerif() *MailVerifQuery {
	query := (&MailVerifClient{config: _q.config}).Query()
...
// WithMailVerif tells the query-builder to eager-load the nodes that are connected to
// the "mail_verif" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithMailVerif(opts ...func(*MailVerifQuery)) *UserQuery {
	query := (&MailVerifClient{config: _q.config}).Query()
...
func (_q *UserQuery) loadMailVerif(ctx context.Context, query *MailVerifQuery, nodes []*User, init func(*User), assign func(*User, *MailVerif)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(mailverif.FieldUserID)
	}
	query.Where(predicate.MailVerif(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.MailVerifColumn), fks...))
	}))
