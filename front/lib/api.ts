const API_BASE = process.env.NEXT_PUBLIC_BASE_URL || "https://localhost:4443";
...
function formatApiError(parsed: Record<string, unknown>, fallback: string): string {
	if (Array.isArray(parsed.errors) && parsed.errors.length) {
		...
		if (first) {
			...
			msg = msg.replace(/^expected string to be RFC 5322 email: .*/, "invalid email format").replace(/^expected number >= (\d+)/, "must be >= $1");
			return `${field}: ${msg}`;
		}
	}
...
export const api = {
	...
	getUrl: (path: string): string => `${API_BASE}${path}`,

	getBlob: async (path: string): Promise<Blob | undefined> => {
