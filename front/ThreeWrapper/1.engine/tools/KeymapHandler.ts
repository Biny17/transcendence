...
import { KeyAction, DEFAULT_KEYBINDINGS } from 'shared/config'
export const DEFAULT_KEYBINDS: KeyBinding[] = (Object.keys(DEFAULT_KEYBINDINGS) as KeyAction[]).map(
  (action) => ({ action, key: DEFAULT_KEYBINDINGS[action] })
)
