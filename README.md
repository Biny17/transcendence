...
| 🍒 **Reverse Proxy** | nginx (alpine) | TLS termination and path-based routing — `/` → frontend, `/api` → Go API, `/server/` → game server |
| 🥭 **Containerization** | Docker Compose | Single-command deployment for all services (frontend, game server, API, database, nginx) |
...
|---|---|---|
| 🍒 `nginx` | **4443 (HTTPS)** | Reverse proxy — single entry point for all traffic |
| 🥭 `web` (frontend) | internal only | Next.js dev server — reachable via `https://<host>/` |
| 🍓 `game-server` | internal only | Bun WebSocket game server — reachable via `https://<host>/server/` |
| 🍌 `api` (backend) | internal only | Go REST API — reachable via `https://<host>/api` |
| 🍎 `db` | `${DB_PORT}` → 5432 | PostgreSQL 17 |

All HTTP traffic is routed through nginx. 🍊 The `web`, `game-server`, and `api` containers are not exposed directly to the host — only nginx is.
...
The API is documented via OpenAPI/Swagger at `https://<host>/api/docs`. 🥭 All routes are served through nginx and prefixed with `/api`. Below are the implemented routes:
